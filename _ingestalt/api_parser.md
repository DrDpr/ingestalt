...
-   `lib/drdpr-horizon/lib/parser.ts` - Main parser implementation
...
-   **Writes to:** `db_horizon` - Stores parsed nodes and edges
    
-   **Used by:** `api_ingest` - Ingestion pipeline
    
-   **Used by:** `api_seed` - Database seeding
    
-   **Implements:** `standards_core` - Node structure standards
