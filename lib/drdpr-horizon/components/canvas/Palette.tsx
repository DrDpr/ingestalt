...
export function Palette() {
  ...
  // Extract all definitions from all standard nodes with unique keys
  ...
  return (
    <div 
      ...
      className="absolute left-6 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-2 p-2 bg-card/40 backdrop-blur-2xl border border-border/10 rounded-2xl shadow-md"
    >
      ...
      <div className="flex flex-col gap-3">
        {availableTypes.map((type) => (
          ...
          >
            ...
            {/* Tooltip */}
            <div className="absolute left-16 px-2 py-1 rounded bg-black text-xs text-foreground whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity border border-border/10 uppercase tracking-widest font-bold">
              {type.title}
            </div>
      ...
      {availableTypes.length === 0 && (
        <div className="w-10 h-10 rounded-xl bg-secondary/20 border border-dashed border-border/5 flex items-center justify-center">
          <span className="text-xs text-foreground/40">Empty</span>
        </div>
      )}
    </div>
  );
