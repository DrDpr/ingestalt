...
function BaseNodeInner({ id, data, icon, colorClass, isStatic, style }: BaseNodeProps) {
  ...
  return (
    <div 
      className={cn(
        "group relative rounded-2xl border p-4 min-w-[200px] shadow-2xl backdrop-blur-xl transition-all duration-300",
        colorClass,
        isSelected ? "ring-2 ring-white/50 scale-105" : "hover:border-white/20",
    ...
    >
      {!isStatic && (
        <button 
          ...
          className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-red-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg hover:bg-red-600 z-50"
        >
          <X size={12} />
        </button>
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-xs uppercase text-white/90 truncate">
            {data.label}
          </div>
          <div className="flex gap-1 mt-1 flex-wrap">
            {data.tags?.slice(0, 3).map((tag: string) => (
              <span key={tag} className="text-[8px] px-1.5 py-0.5 rounded-full bg-white/10 text-white/60 font-medium whitespace-nowrap">
                #{tag}
              </span>
            ))}
          </div>
